function [T,plimix]=E_stepMixs(theta,th_idx,P,G,X,idxVar,type,pairsindex)
%
%   Author
%       Monia Ranalli
...
% compute the E-step of the EM like algorithm for models 9-12
...
if (type==9),
    for g=1:G,
        Sig(:,:,g)=L*diag(theta.l0(g,:))*L'+diag(repmat(psi,[P,1]));
    end
elseif (type==10),
    for g=1:G,
        Sig(:,:,g)=L*diag(theta.l0(g,:))*L'+diag(psi);
    end
elseif (type==11),
    for g=1:G,
        Sig(:,:,g)=L*diag(theta.l0(g,:))*L'+diag(repmat(psi(g),[P,1]));
    end
elseif (type==12),
    for g=1:G,
        Sig(:,:,g)=L*diag(theta.l0(g,:))*L'+diag(psi(:,g));
    end
end
